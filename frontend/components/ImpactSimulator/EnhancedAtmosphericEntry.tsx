...
export function EnhancedAtmosphericEntry({
    ...
    return new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // Temperature gradient (hotter at core, cooler at edges)
          float localTemp = temperature * (0.5 + vIntensity * 0.5) * (0.8 + combinedTurbulence * 0.2);
          
          // Color from blackbody radiation
          vec3 thermalColor = blackbody(localTemp);
          
          // Plasma ionization glow (cyan/blue tint for very hot areas)
          if (localTemp > 5000.0) {
  ...
  useFrame((state, delta) => {
    ...
    if (asteroidRef.current) {
      // Sadece animasyon çalışırken dönüş (roll)
  ...
  return (
    <group ref={groupRef}>
      ...
      {/* Ablation Particles (Yüzey buharlaşması) */}
      {progress > 0.3 && (
