...
export class MemeComposer {
  ...
  static async renderAtmosphericEntry(velocity: number, angle: number): Promise<string> {
    ...
    // Asteroid - boyut ve hız bazlı ışıma
    ...
    // Plazma shield - giriş sırasında oluşan ısı kalkanı
    ...
    // Plazma trail - hız ve açıya göre
  ...
  static async renderThermalFireball(radius: number, temperature: number): Promise<string> {
    ...
    // Gerçek sıcaklık ve yarıçap bazlı boyutlandırma
    ...
    // Volumetric fireball - gerçek boyutta
    ...
    // Dış glow katmanları - yarıçap bazlı
    for (let i = 0; i < 3; i++) {
    ...
    // Heat distortion particles - sıcaklığa göre yoğunluk
  ...
  static async renderShockwave(radius: number): Promise<string> {
    ...
    // Şok dalgası halkaları - gerçek hasar zonlarını temsil ediyor
    // 20psi, 10psi, 5psi, 1psi zones
    const waveCount = 7;
