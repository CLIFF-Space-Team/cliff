"""
Sentry Ingestor
- CNEOS Sentry risk listesini çeker ve veritabanına işler
"""
...
async def ingest_sentry_once() -> Dict[str, Any]:
    ...
    try:
        ...
        for item in items:
            # Sentry alanları: des, ip, n_imp, ps (Palermo), ts (Torino), diameter, h (magnitüd), v_inf
            des = _get_any(item, "des", "id", "objectId")
            if not des:
                continue

            # Sentry'den çap ve H değerlerini al
            ...
            # Etki enerjisini hesapla (çap varsa)
            energy_mt = None
            if diameter_km and impact_probability:
                # Basit: E ≈ (1/2) × kütle × hız² (hız ~20 km/s varsayımı)
